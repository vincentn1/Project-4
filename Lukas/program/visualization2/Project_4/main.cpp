...
void storespinlattice(int **spinArray,int latticesize);
ofstream ofile,Zieldatei;
...
int main()
{
    ...
    for (temp=tempStart; temp<=tempMax; temp+=tempStep){
        ...
        storespinlattice(spinArray,size);
...
void storespinlattice(int **spinArray,int latticesize){
    Zieldatei.open("Datennice.txt");
    for(int j=0;j<latticesize;j++){
        for(int i=0;i<latticesize;i++){
             Zieldatei << spinArray[i][j]<<" ";
        }
        Zieldatei<<endl;
    }
    Zieldatei.close();
