...
int main()
{
    ...
    double**  total_averages[5][mccycles];
    clock_t start, finish;
    start = clock();

//im parallizing the 4 threads each thread run throug specfic temprature part.
#pragma omp parallel sections shared (total_averages)
{
    #pragma omp section
    {
     JohansenCode( tempStart, tempMax/4, tempStep, size, mccycles, isRandomSetup);
    }
    #pragma omp section
    {
     JohansenCode( tempMax/4, 2*tempMax/4, tempStep, size, mccycles, isRandomSetup);
    }
    #pragma omp section
    {
     JohansenCode( 2*tempMax/4, 3*tempMax/4, tempStep, size, mccycles, isRandomSetup);
    }
    #pragma omp section
    {
     JohansenCode( 3*tempMax/4, tempMax, tempStep, size, mccycles, isRandomSetup);
    }
}
...
void initialize(int size, int** grid, bool isRandom, long idum, double& M, double& E)
{
    ...
    if(isRandom){
        ...
        for(i=0; i<size; i++){
            for(j=0; j<size; j++){
                if(ran2(&idum)>0.5){grid[i][j]=-1;}else{grid[i][j]=1;}
            }}}else{
        //all spins up!
        for(i=0; i<size; i++){
            for(j=0; j<size; j++){
